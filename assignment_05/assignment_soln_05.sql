...
--Enrollment table.
SELECT 
STUDENT.STUDENT_ID ,STUDENT.NAME,ENROLLMENT.COURSE_ID FROM STUDENT
...
SELECT
STUDENT.NAME , ENROLLMENT.SEMESTER  FROM STUDENT
INNER JOIN ENROLLMENT ON STUDENT.STUDENT_ID = ENROLLMENT.STUDENT_ID;

--Display student Name and Grade obtained in each course.
SELECT
STUDENT.NAME , ENROLLMENT.GRADE FROM STUDENT
INNER JOIN ENROLLMENT ON STUDENT.STUDENT_ID=ENROLLMENT.STUDENT_ID;
...
--the course.
SELECT
C.COURSE_NAME,C.CREDITS,FACULTY.NAME FROM COURSE C
...
INNER JOIN FACULTY ON FACULTY.FACULTY_ID = COURSE.FACULTY_ID WHERE FACULTY.FACULTY_ID =501;

--Display faculty Name and Designation along with the CourseName they teach.
SELECT 
FACULTY.NAME,FACULTY.DESIGNATION,COURSE.COURSE_NAME FROM FACULTY 
INNER JOIN COURSE ON FACULTY.FACULTY_ID=COURSE.FACULTY_ID;
...
SELECT
S.NAME,C.COURSE_NAME FROM STUDENT  S
INNER JOIN ENROLLMENT  E ON S.STUDENT_ID = E.STUDENT_ID
INNER JOIN COURSE  C ON E.COURSE_ID = C.COURSE_ID;

--Display student Name, CourseName, and Semester.
SELECT
S.NAME,C.COURSE_NAME,E.SEMESTER FROM STUDENT  S
INNER JOIN ENROLLMENT  E ON S.STUDENT_ID = E.STUDENT_ID
INNER JOIN COURSE  C ON E.COURSE_ID = C.COURSE_ID;
...
SELECT 
D.DEPT_NAME,COUNT(S.STUDENT_ID) AS TOTAL_STUDENTS FROM DEPARTMENT D 
JOIN STUDENT S ON D.DEPT_ID = S.DEPT_ID GROUP BY D.DEPT_NAME;

--Display course name along with maximum grade awarded in that course.
SELECT
    C.COURSE_NAME, MAX(E.GRADE) AS MAX_GRADE FROM COURSE C
